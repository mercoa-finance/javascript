...
export class EntityGroup {
    ...
    public async getAll(
        ...
        if (_response.ok) {
            return await serializers.EntityGroupFindResponse.parseOrThrow(_response.body, {
    ...
    public async create(
        ...
        const _response = await core.fetcher({
            ...
            contentType: "application/json",
            body: await serializers.EntityGroupRequest.jsonOrThrow(request, { unrecognizedObjectKeys: "strip" }),
            timeoutMs: requestOptions?.timeoutInSeconds != null ? requestOptions.timeoutInSeconds * 1000 : 60000,
            maxRetries: requestOptions?.maxRetries,
            abortSignal: requestOptions?.abortSignal,
        });
        if (_response.ok) {
            return await serializers.EntityGroupResponse.parseOrThrow(_response.body, {
                unrecognizedObjectKeys: "passthrough",
                allowUnrecognizedUnionMembers: true,
                allowUnrecognizedEnumValues: true,
                breadcrumbsPrefix: ["response"],
    ...
    public async get(
        ...
        const _response = await core.fetcher({
            url: urlJoin(
                (await core.Supplier.get(this._options.environment)) ?? environments.MercoaEnvironment.Production,
                `entityGroup/${encodeURIComponent(await serializers.EntityGroupId.jsonOrThrow(entityGroupId))}`
            ),
            method: "GET",
            headers: {
                Authorization: await this._getAuthorizationHeader(),
