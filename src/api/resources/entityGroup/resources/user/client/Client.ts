...
export class User {
    ...
    public async find(
        ...
        const _response = await core.fetcher({
            url: urlJoin(
                ...
                `/entityGroup/${encodeURIComponent(await serializers.EntityGroupId.jsonOrThrow(entityGroupId))}/users`
            ),
            method: "GET",
            headers: {
        ...
        if (_response.ok) {
            return await serializers.FindEntityGroupUserResponse.parseOrThrow(_response.body, {
    ...
    public async create(
        ...
        const _response = await core.fetcher({
            url: urlJoin(
                (await core.Supplier.get(this._options.environment)) ?? environments.MercoaEnvironment.Production,
                `/entityGroup/${encodeURIComponent(await serializers.EntityGroupId.jsonOrThrow(entityGroupId))}/user`
            ),
            method: "POST",
            headers: {
                Authorization: await this._getAuthorizationHeader(),
            ...
            contentType: "application/json",
            body: await serializers.EntityGroupUserRequest.jsonOrThrow(request, { unrecognizedObjectKeys: "strip" }),
            timeoutMs: requestOptions?.timeoutInSeconds != null ? requestOptions.timeoutInSeconds * 1000 : 60000,
            maxRetries: requestOptions?.maxRetries,
            abortSignal: requestOptions?.abortSignal,
        });
        if (_response.ok) {
            return await serializers.EntityGroupUserResponse.parseOrThrow(_response.body, {
                unrecognizedObjectKeys: "passthrough",
                allowUnrecognizedUnionMembers: true,
                allowUnrecognizedEnumValues: true,
                breadcrumbsPrefix: ["response"],
